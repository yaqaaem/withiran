...
async function handleCreateSignature(request, env) {
  ...
  if (!displayName || !country) return badRequest('نام و کشور الزامی هستند.');

  const ipHash = await sha256Hex(getClientIp(request));
  const duplicate = await env.DB.prepare(
    `SELECT id FROM signatures WHERE ip_hash = ? AND created_at >= datetime('now', '-30 seconds') ORDER BY id DESC LIMIT 1`
  ).bind(ipHash).first();

  if (duplicate) return badRequest('ثبت تکراری بسیار سریع مجاز نیست.', 429);

  await env.DB.prepare(
    `INSERT INTO signatures (display_name, country, message, ip_hash, build_version, created_at)
     VALUES (?, ?, ?, ?, ?, datetime('now'))`
  ).bind(displayName, country, message, ipHash, buildVersion).run();

  return json({ ok: true, message: 'امضا با موفقیت ثبت شد.' });
}
...
async function handleGetSignatures(request, env) {
  ...
  const result = await env.DB.prepare(
    `SELECT id, display_name, country, message, created_at
     FROM signatures ORDER BY datetime(created_at) DESC, id DESC LIMIT ?`
  ).bind(limit).all();
...
async function handleGetStats(env) {
  const result = await env.DB.prepare(
    `SELECT COUNT(*) as total_signatures, COUNT(DISTINCT country) as countries_count FROM signatures`
  ).first();
  return json({ ...(result || { total_signatures: 0, countries_count: 0 }), generated_at: new Date().toISOString() });
}
...
export default {
  async fetch(request, env) {
    try {
      ...
      if (pathname === '/health' && request.method === 'GET') return json({ ok: true, status: 'healthy', time: new Date().toISOString() });
      if (pathname === '/api/sign' && request.method === 'POST') return handleCreateSignature(request, env);
      if (pathname === '/api/signatures' && request.method === 'GET') return handleGetSignatures(request, env);
